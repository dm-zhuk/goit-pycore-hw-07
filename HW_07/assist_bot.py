...
@input_error
def change_contact(name, old_phone, new_phone, book: AddressBook):
    record = book.find(name)
    if record is None:
...
def show_phone(args, book: AddressBook):
    if len(args) != 1:
        raise ValueError("Give me a name please")
    name = " ".join(args)
    record = book.find(name)
    if record:
        return (
    ...
    raise KeyError("Sorry, contact not found")
...
def show_birthday(args, book: AddressBook):
    ...
    name = args[0]
    record = book.find(name)
    return record.get_birthday_str() if record else "Sorry, contact not found"
